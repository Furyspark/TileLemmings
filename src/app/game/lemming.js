...
Game_Lemming.prototype.update = function() {
  ...
  if(this.physicsEnabled) {
    let moveData = {};
    if(!this.dead) moveData = this.preMove(moveData);
    if(!this.dead) moveData = this.move(moveData);
    if(!this.dead) moveData = this.postMove(moveData);
  }
};

Game_Lemming.prototype.updateAnimation = function() {
...
Game_Lemming.prototype.preMove = function(moveData) {
  ...
  if(defaultAction) {
    // Check for ground
    let col = this.map.tileCollision(this.x + this.offsetPoint.floor.x, this.y + this.offsetPoint.floor.y, this);
    if(col === Game_Tile.COLLISION_PASSABLE) {
      // Fall
      if(this.action.current === Game_Lemming.ACTION_WALK) moveData.startFall = true;
      this.onGround = false;
...
Game_Lemming.prototype.move = function(moveData) {
...
Game_Lemming.prototype.postMove = function(moveData) {
  ...
  if(this.map.tileCollision(this.x, this.y, this) === Game_Tile.COLLISION_IMPASSABLE) {
    ...
    if(this.map.tileCollision(this.x + this.offsetPoint.slopeCheck.x, this.y + this.offsetPoint.slopeCheck.y) === Game_Tile.COLLISION_PASSABLE &&
      ...
      if(defaultAction) {
        ...
        moveData.movedUpSlope = true;
      }
    }
    else {
      defaultAction = true;
      // Check whether not to stop climbing
  ...
  // Turn around
  if(defaultAction && moveData.movedUpSlope !== true) {
    this.x -= this.velocity.x;
    this.y -= this.velocity.y;
    this.changeDirection();
  }
  // React to slope downwards
  if(moveData.startFall === true &&
    this.map.tileCollision(this.x + this.offsetPoint.downSlopeCheck.x, this.y + this.offsetPoint.downSlopeCheck.y) === Game_Tile.COLLISION_IMPASSABLE) {
    if(this.offsetPoint.down.x > 0) this.x = (((this.x + this.offsetPoint.downSlopeCheck.x) >> 4) << 4) + this.offsetPoint.moveOutOfGround.x;
    else if(this.offsetPoint.down.x < 0) this.x = (((this.x + 16 + this.offsetPoint.downSlopeCheck.x) >> 4) << 4);
    if(this.offsetPoint.down.y > 0) this.y = (((this.y + this.offsetPoint.downSlopeCheck.y) >> 4) << 4) + this.offsetPoint.moveOutOfGround.y;
    else if(this.offsetPoint.down.y < 0) this.y = (((this.y + 16 + this.offsetPoint.downSlopeCheck.y) >> 4) << 4);
    this.walk();
  }
  return moveData;
}
