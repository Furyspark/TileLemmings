...
ipcMain.on("window", function(event, args) {
  ...
  else if(args[0].toUpperCase() === "MAXIMIZE" && mainWindow != null) {
    mainWindow.maximize();
  }
  else if(args[0].toUpperCase() === "FULLSCREEN" && mainWindow != null) {
    mainWindow.setFullScreen(args[1]);
  }
});
...
function startApp() {
  ...
  mainWindow.on('close', function() {
    ...
    config.window.maximized  = mainWindow.isMaximized();
    config.window.fullscreen = mainWindow.isFullScreen();
